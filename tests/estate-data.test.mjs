...
test('period totals are partial and completions are separate from occupancy',()=>{const s=supplyFor(seed,'울산광역시','전체',2026,'분양');assert.equal(s.value,775);assert.equal(s.partial,true);assert.equal(supplyFor(seed,'울산광역시','전체',2026,'준공').value,1557);assert.equal(supplyFor(seed,'울산광역시','전체',2026,'입주').value,1430);assert.equal(supplyFor(seed,'울산광역시','울주군',2026,'입주').value,1430);assert.equal(supplyFor(seed,'울산광역시','전체',2025,'준공').value,4047);});
